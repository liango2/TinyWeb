...
(fact "When a controller throws an ex other than a ControllerException, Tinyweb returns a 500 status code."
      (let [tw (tinyweb request-handlers [])
            http-request {:path "/runtime-error-controller" :body ""}]
        (tw http-request) => {:status-code 500 :body ""}))

(fact "When view code throws an ex, a 500 response is returned."
      (let [tw (tinyweb request-handlers [])
